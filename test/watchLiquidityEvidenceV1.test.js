...
test('12 EQ point-in-time identity mapping',function(){var c=candidate({sourceId:'EQ1',sourceType:'EQL',eqPartnerProvenance:{currentPivot:{id:'P1'},historicalPartners:[{id:'Z1'},{id:'Z2'}]}}),x=built(c);assert.strictEqual(x.liquidity.eqObjectId,'EQ1');assert.deepStrictEqual(x.liquidity.canonicalSwingIds,[]);assert.strictEqual(x.allCandidates[0].eqCurrentPivotId,'P1');assert.deepStrictEqual(x.allCandidates[0].eqHistoricalPartnerIds,['Z1','Z2']);});
...
test('25 no nearest-Swing guessing',function(){var c=candidate({sourceId:'EQH:1',sourceType:'EQH',side:'BSL'}),x=built(c);assert.strictEqual(x.liquidity.canonicalSwingId,null);assert.deepStrictEqual(x.liquidity.canonicalSwingIds,[]);});
