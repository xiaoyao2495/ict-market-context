...
test('16 removed PDH renders raw identity without calendar label', function () { var s=liquiditySection(render(watch('BEARISH','PDH',nonSwingContext()),true));assert.ok(s.includes('PDH'));assert.ok(!/前一日高点|类型：日线流动性/.test(s));assert.ok(!/周期层级|结构角色/.test(s)); });
test('17 removed PDL renders raw identity without calendar label', function () { var s=liquiditySection(render(watch('BULLISH','PDL',nonSwingContext()),true));assert.ok(s.includes('PDL'));assert.ok(!/前一日低点|类型：日线流动性/.test(s)); });
...
test('28 Beijing notification time is preserved', function () { assert.ok(render(watch('BULLISH','SWING_LOW',swingContext('LOW','LOCAL',['5m'])),true,Date.parse('2026-08-25T17:30:00Z')).includes('时间：08-26 01:30 (UTC+8)')); });
