...
function run(candle, extra) { var cs=base(); cs.push(candle); return detector.detectSingleCandleDisplacement(cs, Object.assign({}, OPTS, extra || {})); }
...
test('explicit price-only contract creates bullish displacement', function(){
  ...
  assert.equal(ev.source,'SINGLE_CANDLE_A'); assert.equal(ev.formationType,'SINGLE_CANDLE');
  assert.equal(ev.startIndex,20); assert.equal(ev.endIndex,20);
  assert.ok(ev.metrics.rangeAtr >= 1.2); assert.ok(ev.metrics.bodyAtr >= 0.8);
  assert.ok(ev.metrics.bodyRatio >= 0.6); assert.ok(ev.metrics.closeExtremeRatio >= 0.75);
});
...
test('doji, open candle, and insufficient ATR are excluded', function(){
  ...
  assert.equal(detector.detectSingleCandleDisplacement([m5(1,5,0,4,1)],OPTS).length,0);
});
test('identity and metadata contain no legacy score or structure fields', function(){
  ...
  assert.equal(ev.id,'BTCUSDT:5m:RAW_DISPLACEMENT:SINGLE_CANDLE:BULLISH:7000000:7299999');
});
test('arbitrary legacy-shaped arguments cannot affect identical candles', function(){
  ...
  var none=detector.detectSingleCandleDisplacement(cs,OPTS);
  var bullish=detector.detectSingleCandleDisplacement(cs,Object.assign({mssEvents:[{id:'M1',direction:'BULLISH',candleIndex:20}]},OPTS));
  var bearish=detector.detectSingleCandleDisplacement(cs,Object.assign({mssEvents:[{id:'M2',direction:'BEARISH',candleIndex:20}]},OPTS));
...
test('prefix output is immutable when future candles are appended', function(){
  var cs=base(); cs.push(m5(100,140,98,138,7000000)); var prefix=detector.detectSingleCandleDisplacement(cs,OPTS);
  cs.push(m5(138,180,137,178,7300000)); var full=detector.detectSingleCandleDisplacement(cs,OPTS).filter(function(e){return e.confirmedAt<=7299999;});
