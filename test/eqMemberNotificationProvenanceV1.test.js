...
function partner(id,price,occurredAt){return{id:id,source:'CAUSAL_ATR50_ZIGZAG',price:price,occurredAt:occurredAt,confirmedAt:occurredAt+300000};}
function watch(type,partners){
    ...
    if(partners!==undefined)p.eqPartnerProvenance={eqModelVersion:'ATR50_36H_UNVIOLATED_CROSS_SOURCE_V1',asOf:30,currentPivot:{id:'P',price:100,occurredAt:20,confirmedAt:30},partnerCount:partners.length,historicalPartners:partners};
...
test('EQH displays current 2/2 and ATR50 historical partners',function(){var s=build(watch('EQH',[partner('A',100.1,Date.parse('2026-08-27T20:05:00Z')),partner('B',99.9,Date.parse('2026-08-27T21:10:00Z'))]));assert.ok(s.includes('EQ 当前点：2/2 @ 100.0'));assert.ok(s.includes('ATR50 历史配对：2 个'));assert.ok(s.includes('历史点位：100.1 / 99.9'));assert.ok(s.includes('历史时间（北京时间）：08/28 04:05 / 08/28 05:10'));});
test('EQL uses the same non-cluster point-in-time wording',function(){var s=build(watch('EQL',[partner('A',90,10)]));assert.ok(s.includes('ATR50 历史配对：1 个'));assert.ok(!s.includes('EQ 构成'));assert.ok(!s.includes('成员'));});
...
test('non-EQ notification has no EQ pairing line',function(){var w=watch('EQH',[partner('A',1,1)]);w.liquidityTaken.primary.sourceType='PDH';delete w.liquidityTaken.primary.eqPartnerProvenance;var s=build(w);assert.ok(!s.includes('EQ 配对'));assert.ok(!s.includes('ATR50 历史配对'));});
test('more than six partners truncates presentation only',function(){var ps=[];for(var i=0;i<7;i++)ps.push(partner(String(i),i,10+i));var s=build(watch('EQL',ps));assert.ok(s.includes('… 共 7 个'));assert.ok(s.includes('ATR50 历史配对：7 个'));});
