...
test('EQ keeps object identity and all as-of member contexts without primary selection',function(){var p=makeProjector(),x=adapter.buildSweepContextV1(candidate('EQ1','EQL',80),options(p));assert.equal(x.contextApplicability,'EQ_MULTI_MEMBER');assert.equal(x.canonicalSwingId,null);assert.deepEqual(x.memberSwingContexts.map(function(c){return c.canonicalSwingId;}),['L','L2']);assert.equal(x.provenance.singleMemberPrimarySelected,false);});
test('later EQ member cannot rewrite an earlier SweepContext',function(){var p=makeProjector(),x=adapter.buildSweepContextV1(candidate('EQ1','EQL',60),options(p));assert.deepEqual(x.memberSwingContexts.map(function(c){return c.canonicalSwingId;}),['L']);});
