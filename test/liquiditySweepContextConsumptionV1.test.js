...
test('explicit liquidity preserves native identity and never guesses a Swing',function(){var x=adapter.buildSweepContextV1(candidate('BTC:ASIA_HIGH:2026-01-01','ASIA_HIGH',50),{});assert.equal(x.contextApplicability,'NON_SWING_LIQUIDITY');assert.equal(x.liquidity.sourceId,'BTC:ASIA_HIGH:2026-01-01');assert.equal(x.canonicalSwingId,null);assert.equal(x.swingContext,null);assert.equal(x.provenance.nearestSwingGuessing,false);});
