...
function lightweightService(symbol, source, request, extra) {
    return biasV3.createService(Object.assign({symbol:symbol,getFourHourCandles:function(){return source;},
        buildFacts:function(cs,at){return compactFacts(symbol,at);},requestSemantic:request||function(){return Promise.resolve(decision());}},extra||{}));
}
...
test('30 available notification presentation is concise and hides facts/version names', function () {var b={status:'AVAILABLE',semantic:decision(),facts:{adx14:99}},text=context.lines(b).join('\n');assert.match(text,/📊 4H Bias/);assert.match(text,/方向:/);assert.doesNotMatch(text,/adx14|V1|V2|V3|FACT_SET/);});
...
test('41 observation log is one flat structured record per new 4H snapshot', async function () {var cs=candles(2),records=[],svc=lightweightService('BTCUSDT',cs,null,{observe:function(record){records.push(record);},now:function(){return 12345;}}),first=await svc.refresh(cs[1].closeTime);await svc.refresh(cs[1].closeTime+1000);assert.equal(records.length,1);assert.deepEqual(Object.keys(records[0]).sort(),['buildDurationMs','closedAt','confidence','conflicts','direction','facts','generatedAt','llmDurationMs','promptHash','status','strength','summary','symbol']);assert.equal(records[0].generatedAt,first.generatedAt);assert.equal(records[0].direction,'BULLISH');assert.equal(records[0].strength,'MODERATE');assert.equal(records[0].confidence,'HIGH');assert.deepEqual(records[0].facts,first.facts);});
