...
test('V3 EQ Sweep reaches liquidityTaken using V3 cluster ID',function(){var f=fixture(),c=candle(23,95,101,99),event=lifecycle.evaluateLiquidity(f.cluster,c);Object.assign(f.cluster,event);var sweep=sweepAdapter.buildSweepEvent(f.cluster,c,23);var leg={ids:['D1'],direction:'BEARISH',startIndex:24,lastIndex:24,endIndex:24,firstConfirmedAt:c.closeTime+1,lastConfirmedAt:c.closeTime+1};var watch=displacementWatch.buildWatch({symbol:'BTCUSDT',leg:leg,evaluationTime:c.closeTime+1000,sweepEvents:[sweep],displacements:[],mssEvents:[],candles:candles});assert.ok(watch);assert.strictEqual(watch.liquidityTaken.primary.sourceId,f.cluster.id);assert.strictEqual(watch.liquidityTaken.primary.eqMemberProvenance.memberCount,2);});
test('ordinary Swing remains excluded from Narrative WATCH candidates',function(){var f=fixture(),c=candle(23,95,101,99),s=sweepAdapter.buildSweepEvent(f.a,c,23),leg={ids:['D1'],direction:'BEARISH',startIndex:24,lastIndex:24,endIndex:24,firstConfirmedAt:c.closeTime+1,lastConfirmedAt:c.closeTime+1};assert.strictEqual(displacementWatch.buildWatch({symbol:'BTCUSDT',leg:leg,evaluationTime:c.closeTime+1000,sweepEvents:[s],candles:candles}),null);});
